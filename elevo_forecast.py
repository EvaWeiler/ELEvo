...
header = r'ID, time 21.5 [UT, at 21.5 R_Sun], lon [deg], lat [deg], half width [deg], initial speed [km/s], arrival time [UTC], error arrival time [h], arrival speed [km/s], error arrival speed [km/s], arrival probability [%]'
...
def donki_kinematics(i):
    ...
    # Ensemble sampling
    data_longitude = np.random.normal(data.longitude[i], 5, n_ensemble)
    
    data_longitude_rad = np.deg2rad(data_longitude)
    halfwidth_ens = np.random.normal(halfwidth, 5*np.pi/180, n_ensemble)  # 5 deg in rad

    # Earth position
    dct = t0_num[i] - earth.time
    earth_ind = np.argmin(np.abs(dct))
    earth_lon = earth.lon[earth_ind] * np.ones(n_ensemble)

    # Calculate longitudinal separation
    delta_earth = data_longitude_rad - earth_lon
    wrap_mask = (np.abs(data_longitude_rad) + np.abs(earth_lon) > np.pi) & \
                (np.sign(data_longitude_rad) != np.sign(earth_lon))
    delta_earth[wrap_mask] = data_longitude_rad[wrap_mask] - (earth_lon[wrap_mask] + 2 * np.pi * np.sign(data_longitude_rad[wrap_mask]))

    # Hit test
    cme_hit = np.zeros(n_ensemble, dtype=int)
    cme_hit[np.abs(delta_earth) < halfwidth_ens] = 1
    
    # Probability of hit
    prob_hit = np.sum(cme_hit) / n_ensemble
    ...
    cme_delta=np.mean(delta_earth,axis=0)*np.ones([kindays_in_min,3])
...
def donki_kinematics_new(i):
    ...
    # Hit test
    cme_hit_ens = np.zeros(n_ensemble, dtype=int)
    cme_hit_ens[np.abs(delta_earth) < halfwidth_ens] = 1
    
    # Probability of hit
    prob_hit = (np.sum(cme_hit_ens) / n_ensemble)*100.
    ...
    cme_r_ensemble=np.zeros([kindays_in_min, n_ensemble])
    cme_v_ensemble=np.zeros([kindays_in_min, n_ensemble])
    cme_delta=np.mean(delta_earth)*np.ones([kindays_in_min,3])
    cme_delta_solo = [[x] * 3 for x in delta_solo_list]
    cme_delta_sta = [[x] * 3 for x in delta_sta_list]
    cme_hit=np.zeros(kindays_in_min)
    cme_hit[np.abs(np.mean(delta_earth,axis=0))<halfwidth] = 1
    cme_hit_solo=np.zeros(kindays_in_min)
    cme_hit_solo[np.abs(delta_solo_list[0])<halfwidth] = 1
    ...
    def process_arrival(distance, obj, time1, cme_v, cme_id, t0, i, cme_lon, cme_lat, data, label):
        ...
        if not np.isnan(distance).all():
            ...
            arr_speed_err_list.append(err_arr_speed / 2)
            if label=='earth': 
                arr_hit_prob.append(prob_hit)
            else:
                arr_hit_prob.append(np.nan)
    ...
    return time2_num, cme_r, cme_lat, cme_lon, cme_a, cme_b, cme_c, cme_id, cme_v, results_earth['arr_time_fin'], results_earth['arr_time_err0'], results_earth['arr_time_err1'], results_earth['arr_id'], results_earth['arr_hit'], results_earth['arr_speed_list'], results_earth['arr_speed_err_list'], results_sta['arr_time_fin'], results_earth['arr_hit_prob']
...
#results = pool.map(donki_kinematics_new, range(2))
...
hc_arr_prob = [result[17] for result in results]
hc_arr_prob1 = np.concatenate(hc_arr_prob)
...
def make_frame(k):
    ...
    if not (hc_arr_id1=='nan').all(): ax0.set_position([0.55, 0.869, 0.36, 0.03])  # First subplot [left, bottom, width, height]
...
fin = pd.read_csv(movie_path+'elevo_arrival_times.txt', skiprows=2, names=['ID', 'time 21.5 [UT, at 21.5 R_Sun]', 'lon [deg]', 'lat [deg]', 'half width [deg]', 'initial speed [km/s]', 'arrival time [UTC]', 'error arrival time [h]', 'arrival speed [km/s]', 'error arrival speed [km/s]', 'arrival probability [%]'], delimiter=' ')
...
header_solo='ELEvo CME arrival times at Solar Orbiter (using heliocentric distance), Austrian Space Weather Office / GeoSphere Austria, created ' + str(today.strftime('%A'))[0:3] + ' ' + date_today_minutes + ' UTC \n'
...
file_sta = movie_path+'elevo_arrival_times_sta.html'
